...
import QrisSettingsModal from "./components/QrisSettingsModal";
...
// Email admin dari env (VITE_ADMIN_EMAIL). UI admin hanya aktif kalau
// email sesi Supabase cocok. Proteksi sesungguhnya tetap di RLS Supabase.
...
function isAllowedAdmin(email, extraList = []) {
  const e = (email || "").trim().toLowerCase();
  if (!e) return false;
  if (e === ADMIN_EMAIL) return true;
  if (ENV_ADMIN_EMAILS.includes(e)) return true;
  return extraList.some((x) => (x || "").trim().toLowerCase() === e);
}

if (!ADMIN_EMAIL) {
  // eslint-disable-next-line no-console
  console.warn(
    "VITE_ADMIN_EMAIL belum di-set. Isi di .env (lokal) dan Vercel Environment Variables (production)."
  );
}

export default function JadwalOlahraga() {
  const [page, setPage] = useState(() => (typeof window !== "undefined" ? pageFromHash() : PAGE_DAILY));
  const keys = kvKeys(page);
  ...
  const isAdmin = isAllowedAdmin(sessionEmail, contributorEmails);
  const isPrimaryAdmin = (sessionEmail || "").toLowerCase() === ADMIN_EMAIL;
  ...
  const [qrisSettingsOpen, setQrisSettingsOpen] = useState(false);
  const [navLabels, setNavLabels] = useState({ daily: "Harian", major: "Event Besar" });
  const [navSettingsOpen, setNavSettingsOpen] = useState(false);
  const [pageHeaders, setPageHeaders] = useState(DEFAULT_HEADERS);
  ...
  const goToPage = (next) => {
    const target = next === PAGE_MAJOR ? PAGE_MAJOR : PAGE_DAILY;
    if (window.location.hash !== hashForPage(target)) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const logos = await getKV("broadcasterLogos");
        if (logos) setCustomLogos(logos);
      } catch (e) {
        /* no custom logos yet, or Supabase belum dikonfigurasi */
      }
      try {
        const qr = await getKV("qrisUrl");
        if (typeof qr === "string") setQrisUrl(qr);
      } catch (e) { /* keep default image */ }
      try {
        const ae = await getKV("adminEmails");
        if (Array.isArray(ae)) {
          setContributorEmails(ae.map((x) => String(x).trim().toLowerCase()).filter(Boolean));
        }
      } catch (e) { /* none */ }
      try {
    ...
    const unsubNav = subscribeKV("navLabels", (value) => {
      if (value && typeof value === "object") {
        setNavLabels({
          daily: value.daily || "Harian",
          major: value.major || "Event Besar",
        });
      }
    });
    return () => {
      unsubBroadcaster();
      if (typeof unsubQris === "function") unsubQris();
      if (typeof unsubAdmins === "function") unsubAdmins();
      if (typeof unsubHeaders === "function") unsubHeaders();
  ...
  const saveEventLogo = async (name, logo) => {
    ...
    setEventLogos(next);
    try {
      await setKV(kvKeys(page).eventLogos, next);
    } catch (e) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      let loaded = [];
      try {
        const r = await getKV(k.events);
        loaded = r || [];
        if (!r) await setKV(k.events, loaded);
      } catch (e) {
        loaded = [];
      }
      ...
      let logos = {};
      let sportList = DEFAULT_SPORTS;
      try {
        const el = await getKV(k.eventLogos);
        if (el) logos = el;
      } catch (e) { /* none */ }
      try {
        const sp = await getKV(k.sports);
        sportList = normalizeSports(sp);
      } catch (e) { /* defaults */ }
    ...
    const k = kvKeys(page);
    const unsubEventLogos = subscribeKV(k.eventLogos, (value) => setEventLogos(value || {}));
    const unsubSports = subscribeKV(k.sports, (value) => setSports(normalizeSports(value)));
    ...
    return () => {
      cancelled = true;
      if (typeof unsubEventLogos === "function") unsubEventLogos();
      if (typeof unsubSports === "function") unsubSports();
  ...
  const saveEvent = async () => {
    ...
    try {
      ...
      const cleanSport = (draft.sport || "").trim();
      let cleaned = {
        ...draft,
        matches: cleanMatches,
        broadcasters: cleanBroadcasters,
        sport: cleanSport,
        pinned: !!draft.pinned,
      };

      // Olahraga baru (tulis sendiri) → masuk daftar popularitas
      if (cleanSport) {
        const nextSports = addSportToList(sports, cleanSport);
        if (nextSports.length !== sports.length || nextSports.some((s, i) => s !== sports[i])) {
          setSports(nextSports);
          try {
            await setKV(kvKeys(page).sports, nextSports);
          } catch (e) {
            /* non-blocking */
          }
        }
  ...
  // 1) pinned  2) popularitas olahraga  3) jam match paling awal  4) order manual
  ...
  const getSortedGroupsForDate = (date) =>
    Object.values(byDate[date]).sort((a, b) => {
      ...
      const sportA = sportRank(a.event.sport || a.sourceEvents[0]?.sport, sports);
      const sportB = sportRank(b.event.sport || b.sourceEvents[0]?.sport, sports);
      if (sportA !== sportB) return sportA - sportB;

      const timeA = earliestMatchKey(a);
      const timeB = earliestMatchKey(b);
  ...
  const saveContributors = async (list) => {
    if (!isPrimaryAdmin) return;
    const next = (list || [])
      .map((x) => String(x).trim().toLowerCase())
      .filter((x) => x && x.includes("@") && x !== ADMIN_EMAIL);
    setSaving(true);
    try {
      await setKV("adminEmails", next);
      setContributorEmails(next);
      setContributorsOpen(false);
      setToast("Akses kontributor disimpan");
      setTimeout(() => setToast(""), 2000);
    } catch (err) {
      setToast(err.message || "Gagal simpan akses");
      setTimeout(() => setToast(""), 3000);
    } finally {
      setSaving(false);
    }
  ...
  const saveQrisUrl = async (url) => {
    if (!isAdmin) return;
    setSaving(true);
    try {
      const value = (url || "").trim();
      await setKV("qrisUrl", value);
      setQrisUrl(value);
      setQrisSettingsOpen(false);
      setToast("Gambar QRIS disimpan");
      setTimeout(() => setToast(""), 2000);
    } catch (err) {
      setToast(err.message || "Gagal simpan QRIS");
      setTimeout(() => setToast(""), 3000);
    } finally {
      setSaving(false);
    }
  ...
  const saveSports = async (list) => {
    if (!isAdmin) return;
    const next = normalizeSports(list);
    setSaving(true);
    try {
      await setKV(kvKeys(page).sports, next);
      setSports(next);
      setSportsModalOpen(false);
      setToast("Urutan olahraga disimpan");
      setTimeout(() => setToast(""), 2000);
    } catch (err) {
      setToast(err.message || "Gagal simpan olahraga");
      setTimeout(() => setToast(""), 3000);
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div style={styles.page}>
      ...
      <Header
        ...
        headerContent={page === PAGE_MAJOR ? pageHeaders.major : pageHeaders.daily}
        isAdmin={isAdmin}
        page={page}
        navLabels={navLabels}
        ...
        onEditNav={() => setNavSettingsOpen(true)}
        onEditHeader={() => setHeaderSettingsOpen(true)}
      ...
      {sortedDates.length === 0 && (
        <div className="jo-content" style={styles.emptyState}>
          {q
            ...
            : page === PAGE_MAJOR
            ? "Belum ada event besar. Tambah event di dashboard ini."
      ...
      <FAB
        ...
        onOpenSports={() => { setFabOpen(false); setSportsModalOpen(true); }}
        onOpenQris={() => { setFabOpen(false); setQrisSettingsOpen(true); }}
        onOpenNav={() => { setFabOpen(false); setNavSettingsOpen(true); }}
        onOpenContributors={() => { setFabOpen(false); setContributorsOpen(true); }}
        onOpenHeader={() => { setFabOpen(false); setHeaderSettingsOpen(true); }}
        onNewEvent={openNewEvent}
      />

      {toast && <div style={styles.toast}>{toast}</div>}
      <QrisSettingsModal
        open={qrisSettingsOpen}
        imageUrl={qrisUrl}
        saving={saving}
        setSaving={setSaving}
        setToast={setToast}
        onSave={saveQrisUrl}
        onClose={() => setQrisSettingsOpen(false)}
      />
      <NavSettingsModal
        open={navSettingsOpen}
        labels={navLabels}
        saving={saving}
        onSave={saveNavLabels}
        onClose={() => setNavSettingsOpen(false)}
      />
      <ContributorsModal
        open={contributorsOpen}
        primaryEmail={ADMIN_EMAIL}
        emails={contributorEmails}
        saving={saving}
      ...
      <HeaderSettingsModal
        ...
        page={page}
        pageLabel={page === PAGE_MAJOR ? (navLabels?.major || "Event Besar") : (navLabels?.daily || "Harian")}
        value={page === PAGE_MAJOR ? pageHeaders.major : pageHeaders.daily}
        fallbackLogo={BRAND_LOGO}
        saving={saving}
        setSaving={setSaving}
        setToast={setToast}
        onSave={savePageHeader}
        onClose={() => setHeaderSettingsOpen(false)}
      />
      <SupportModal imageUrl={qrisUrl} />
    </div>
  );
}
